...
'https://de.2em.ch/autovermietung?adresse=luzern&search=Suche&date_depart=24%2F06%2F2023&date_retour=26%2F06%2F2023&state_am=12%3A00&state_pm=16%3A00&vehicule=auto&latitude=&longitude=&resetsearch=1'
...
files=(
'car_data_bern.json'
'car_data_luzern.json'
'car_data_basel.json'
'car_data_lausanne.json'
'car_data_geneva.json'
'car_data_zurich.json'
)
...
for i in "${!urls[@]}"; do
    url=${urls[i]}
    file=${files[i]}
    python3 scrape_cars.py -u "$url" -o "$file"
done
